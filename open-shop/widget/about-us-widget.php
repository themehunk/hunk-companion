 ...
 */
// add admin scripts
function open_shop_widget_enqueue(){
    wp_enqueue_media();
    wp_enqueue_script('open_shop_widget_script', get_template_directory_uri() . '/js/widget.js', array( 'jquery', 'wp-color-picker' ),false, '1.0', true);
}
add_action('admin_enqueue_scripts', 'open_shop_widget_enqueue');
// register widget
function open_shop_about_us_widget(){
register_widget( 'THunk_aboutme' );
}
add_action('widgets_init','open_shop_about_us_widget');
class THunk_aboutme extends WP_Widget {
    function __construct() {
        $widget_ops = array('classname' => 'th-about-me',
            'description' => 'Display member image with description, link and font awesome icons');
        parent::__construct('th-about-me-widget', __('Open Shop : About Us widget','hunk-companion'), $widget_ops);
    }
...
<p>
    ...
    <p>
        <label for="<?php echo $this->get_field_id('author_img_uri'); ?>"><?php _e('Member Image','hunk-companion'); ?></label></p><P>
        <?php
